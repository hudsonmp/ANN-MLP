...
class MNISTLoader:
    ...
    def __init__(self, subset_size=None, random_state=42):
    ...
    def load_mnist(self):
        ...
        print(
            f"Dataset loaded: {X_train.shape[0]} training samples, {X_test.shape[0]} test samples"
        )
    ...
    # After converting labels to integers
    def prepare_binary_task(self, X_train, X_test, y_train, y_test, digit=0):
        ...
        print(
            f"Positive class (digit {digit}): {sum(y_train_binary)} training samples, {sum(y_test_binary)} test samples"
        )
